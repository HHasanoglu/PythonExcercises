...
class LinkedList:
    ...
    def print_list_forward(self, node, output):
        if node is not None:
            if output != "":
                output += "->"
            output += str(node.value)
            return self.print_list_forward(node.next, output)
        else:
            print(output)


my_linked_list = LinkedList()
for value in [4, 8, 6, 7, 1, 3, 5, 10, 15, 2, 9]:
    my_linked_list.sortedInsert(value)
    my_linked_list.print_list_forward(my_linked_list.head, "")
